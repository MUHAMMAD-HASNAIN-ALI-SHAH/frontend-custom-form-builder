...
const ResponseForm = ({ formId }: { formId: string }) => {
  ...
  const getInputClasses = (index: number) => {
    return `w-full sm:w-[50%] border-0 border-b-2 ${
      form.questions[index]?.error ? "border-red-500" : "border-gray-300"
    ...
    <form onSubmit={(e) => onSubmit(e, formId)} className="w-full">
      ...
      <div className="w-full flex justify-between items-center px-7 py-4 rounded-lg">
        <button
          ...
          type="submit"
          className={`bg-blue-700 cursor-pointer px-3 py-1 text-white rounded-sm hover:bg-blue-800 ${
            submitFormLoader ? "opacity-50 cursor-not-allowed" : ""
          }`}
        >
          Submit
        </button>
